    ...
    while(1){
       ...
       // pattern_code+=Sensor_1();
        ...
        if((ISR==1)&&(pattern_code==0)){ // if detects object and not on white line
           ...
           TIMER_A2->CCR[3]=(6857*50/100); // right wheel moves 50% duty cycle
           TIMER_A2->CCR[4]=(6857*50/100); //  left wheel moves 50% duty cycle
        }
        ...
        if((ISR==0)&&(pattern_code==0)){ // if doesn't detect object and not on white line
           ...
           P3->OUT=0x40; //  set port 3 bit 7 to one to set high ~ move forward
           P2->OUT=0x40; // set port 2 bit 6 to one to set high ~ move forward
           TIMER_A2->CCR[3]=(6857*50/100); // right wheel moves 50% cycle
           TIMER_A2->CCR[4]=(6857*50/100); //  left wheel moves 50% cycle
           for(i=100000;i>0;i--){}  //Lousy Delay

           //Move Forward//
          P3->OUT=0x40; //  set port 3 bit 7 to one to set high ~ move forward
          P2->OUT=0x40; // set port 2 bit 6 to one to set high ~ move forward
          TIMER_A2->CCR[3]=(6857*dutyA/100); // right wheel moves dutyA cycle
          TIMER_A2->CCR[4]=(6857*dutyB/100); //  left wheel moves dutyB cycle
        ...
        if(((ISR==1)||(ISR==0))&&(pattern_code!=0)){ // if on white line
           ...
           if(pattern_code==1){ // if the sensors 4 ~ Front Right Sensors ~ see white
               ...
               dutyA=80;
               dutyB=20;
               for(i=200000;i>0;i--){}  //Lousy Delay
           }
           ...
           if(pattern_code==2){ // if the sensors 3 ~ Front Left Sensors ~ see white
               ...
               dutyA=20;
               dutyB=80;
               for(i=300000;i>0;i--){}  //Lousy Delay
           }
           ...
           if(pattern_code==3){ // if the sensors 3&4 ~ Front Sensors ~ see white
               ...
               TIMER_A2->CCR[3]=(6857*50/100); // right wheel moves 50% duty cycle
               TIMER_A2->CCR[4]=(6857*20/100); //  left wheel moves 20% duty cycl
               for(i=200000;i>0;i--){}  //Lousy Delay
           }
