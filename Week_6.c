...
void main(void){
    ...
    while(1){
               ...
               if(pattern_code==1){ // if the sensors 4 ~ Front Right Sensors ~ see white
                   ...
                   TIMER_A2->CCR[3]=(1714*20/100); // compare value start at 80% duty cycle
                   TIMER_A2->CCR[4]=(1714*20/100); //  @ 80% duty cycle count value
               ...
               if(pattern_code==3){ // if the sensors 3&4 ~ Front Sensors ~ see white
                   ...
                   TIMER_A2->CCR[3]=(1714/2); // compare value start at 50% duty cycle
                   TIMER_A2->CCR[4]=(1714*20/100); //  @ 50% duty cycle count value
                   for(i=100000;i>0;i--){}  //Lousy Delay
               }
...
//P8.2 ~ IR Transmitter//
     ...
     // 0000 00    10    00    01   0   0    1   0
       ...
       TIMER_A3->CCR[0]=(80); // Top Value = Wanted 38KHz square wave
       TIMER_A3->CCR[2]=(40); // 38KHz @ 50% duty cycle count value
...
// 8.2 Handler ~ IR Transmitter //
    static count; //1 count = 26.316us
    count++; // each count takes 26.316us

    if(count==33){ // PWM off for 500us or 5ms;
    ...
    if(count==365){ // PWM on for 600us or 6ms
