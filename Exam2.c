...
void main(void){
    ...
    while(1){
       ...
       TIMER_A2->CCR[3]=(6857*30/100); // compare value start at 50% duty cycle
       TIMER_A2->CCR[4]=(6857*30/100); //  @ 50% duty cycle count value
        } // end do one time

        delay++;

        if (delay<=140){ // for 2 seconds count white lines
            ...
            if((pattern_code==3)){ // if both sensors pass a white line
                white++; // count 1 to white variable
            ...
            if (delay==140){
            LCD_goto_xy(1,3);
            LCD_print_dec3(white); // print white line count on LCD

            // stop //

            TIMER_A2->CCR[3]=(0); // compare value start at 0% duty cycle
            TIMER_A2->CCR[4]=(0); //  @ 0% duty cycle count value
            Delay_48MHz_sec(3); // pause for 3 seconds

            // Move Right 180 //
            P3->OUT=0x40; //  set port 3 bit 6 to one to set high
            P2->OUT=0x80; // set port 2 bit 6 to one to set high

            TIMER_A2->CCR[3]=(6857*40/100); // compare value start at 40% duty cycle
            TIMER_A2->CCR[4]=(6857*40/100); //  @ 40% duty cycle count value
            Delay_48MHz_sec(2); // rotate 180
        ...
        while((delay>140)&&(ISR==0)){}
            if((ISR==1)){ // if object is detected
                //Move Forward//
               P3->OUT=0x40; //  set port 3 bit 7 to one to set high ~ move forward
               P2->OUT=0x40; // set port 2 bit 6 to one to set high ~ move forward
               TIMER_A2->CCR[3]=(6857*20/100); // compare value start at 20% duty cycle
               TIMER_A2->CCR[4]=(6857*20/100); //  @ 20% duty cycle count value
               Delay_48MHz_sec(2);
               TIMER_A2->CCR[3]=(0); // compare value start at 0% duty cycle
               TIMER_A2->CCR[4]=(0); //  @ 0% duty cycle count value
               return 0;
